...
def process_epg(epg_channels, epg_programmes):
...
def main():
    ...
    live_m3u_entries, epg_channels = process_live_channels(device_id)
    process_vod_shows(device_id)
    epg_programmes = fetch_epg_programmes()
    process_epg(epg_channels, epg_programmes)
    
    m3u_lines = ['#EXTM3U x-tvg-url="epg.xml.gz"']
    for extinf, url in live_m3u_entries:
